...
optionList <- list(
    ...
    optparse::make_option(
      ...
      type = "character",
      default = '',
      help = "Pvar file to pull variants"
    ),
...
if (!is.null(arguments$options$null_model_rds)) {
  # If not wald test, run score test and return
  print('Testing SNPs using the SCORE test...')
  
  null_mod <- readRDS(arguments$options$null_model_rds)
  rez <- GMMAT::glmm.score(
    null_mod,
    infile = arguments$options$gds_file,
    outfile = arguments$options$out_file,
    ncores = arguments$options$nthreads
  )
  
} else {
  # Run wald test
  print('Testing SNPs using the Wald test...')
  
  vars <- read.csv(
    arguments$options$variants,
    sep = '\t',
    header = T,
    comment.char = "#"
  )
